...
public:
  ...
  /**
   * Loads the vocabulary from a binary file for ORB2
   * @param filename
   */
  bool loadFromBinaryFile2(const std::string &filename);

  /**
   * Saves the vocabulary into a binary file for ORB2
   * @param filename
   */
  void saveToBinaryFile2(const std::string &filename) const;  
...
template<class TDescriptor, class F>
bool TemplatedVocabulary<TDescriptor,F>::loadFromBinaryFile2(const std::string &filename) {
  std::fstream f;
  f.open(filename.c_str(), std::ios_base::in|std::ios::binary);
  unsigned int nb_nodes, size_node;
  f.read((char*)&nb_nodes, sizeof(nb_nodes));
  ...
  while (!f.eof()) {
    ...
    //m_nodes[nid].parent = *(const int*)buf;
    m_nodes[m_nodes[nid].parent].children.push_back(nid);
    m_nodes[nid].descriptor.resize(F::L);
    memcpy(&m_nodes[nid].descriptor[0],buf+4,F::L);
    m_nodes[nid].weight = *(float*)(buf+4+F::L); // F::L
    if (buf[8+F::L]) { // is leaf //F::L
      int wid = m_words.size();
      m_words.resize(wid+1);
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::saveToBinaryFile2(const std::string &filename) const {
  std::fstream f;
  f.open(filename.c_str(), std::ios_base::out|std::ios::binary);
  unsigned int nb_nodes = m_nodes.size();
  float _weight;
  ...
  for(size_t i=1; i<nb_nodes;i++) {
    const Node& node = m_nodes[i];
    f.write((char*)&node.parent, sizeof(node.parent));
    f.write((char*)&node.descriptor[0], F::L);//F::L
    _weight = node.weight; 
